...
size_t heap_size = 0;
double get_utilization() {
    return (double)( user_malloc_size * 1.0 / heap_size ); // 计算使用率 
}
/* 
...
int mm_init(void)
{
    ...
    heap_listp += (2 * WSIZE);
    heap_size += (4 * WSIZE); // 相当于初始化？测试时发现是必要的
...
void *mm_malloc(size_t size)
{
    ...
    {
        place(bp, newsize);
        user_malloc_size += (GET_SIZE(HDRP(bp)) - WSIZE); // 分配块
        return bp;
    }
    #else
...
void mm_free(void *bp)
{
    ...
    PUT(FTRP(bp), PACK(size, prev_alloc, 0));  // coalease 写法示例
    ...
    user_malloc_size -= (size - WSIZE); // ！！应该放在 coalesce 后面，因为可能有合并空闲块的操作
}
...
static void *extend_heap(size_t words)
{
    ...
    char *bp;
    size_t size;
    size = (words % 2) ? (words + 1) * WSIZE : words * WSIZE; // 双字节对齐
...
static void *coalesce(void *bp)
{
    ...
    */
    ...
    {
        char* next_bp = NEXT_BLKP(bp); // 获取后块指针
        size_t size_next = GET_SIZE(HDRP(next_bp)); // 获取后块大小
        delete_from_free_list(next_bp); // 从空闲列表删去后块

        PUT(HDRP(bp), PACK(size + size_next, 1, 0)); // 合并两个块
        PUT(FTRP(next_bp), PACK(size + size_next, 1, 0));
    }

    else if (!prev_alloc && next_alloc) /*前块空闲，后块已分配*/
    {
        char* prev_bp = PREV_BLKP(bp); // 获取前块指针
        size_t size_prev = GET_SIZE(HDRP(prev_bp)); // 获取前块大小
        delete_from_free_list(PREV_BLKP(bp)); // 从空闲列表删去前块

        size_t prev_alloc = GET_PREV_ALLOC(HDRP(prev_bp)); // 状态

        PUT(HDRP(prev_bp), PACK(size + size_prev, prev_alloc, 0));
        PUT(FTRP(bp), PACK(size + size_prev, prev_alloc, 0));

        bp = prev_bp;  // ！！！ 一定要记得把指针挪到前面去！！！                                              
    }

    else /*前后都是空闲块*/
    {
        char* prev_bp = PREV_BLKP(bp), *next_bp = NEXT_BLKP(bp);
...
static void *find_fit_first(size_t asize)
{
   ...
   char *ptr = free_listp; // 链表头
   while(ptr) {
       if(GET_SIZE(HDRP(ptr)) > asize) return ptr; 
       ptr = (void *) GET_SUCC(ptr); // 获取后记
   }
   return NULL; 
}
...
static void* find_fit_best(size_t asize) {
    /* 
        ...
        HINT: asize 已经计算了块头部的大小
    */
    char *ptr = free_listp, *bst = (void *)NULL;
    int fit = 0;
    int fit_val = 0xfffffff;
    while(ptr) {
        if(GET_SIZE(HDRP(ptr)) > asize) {
            if(fit == 0) fit = 1;
            if(fit_val > GET_SIZE(HDRP(ptr)) - asize){
                fit_val = GET_SIZE(HDRP(ptr)) - asize;
                bst = (void *)ptr;
            }
        }
...
static void place(void *bp, size_t asize)
{
    ...
    */
    ...
    if(prev_size - asize > MIN_BLK_SIZE) {
        ...
        PUT(HDRP(next_bp), PACK(prev_size - asize, 1, 0)); // 设置新的空闲块（放回去）
        PUT(FTRP(next_bp), PACK(prev_size - asize, 1, 0));

        add_to_free_list(next_bp);
    } else {
        // 如果空闲块多余的空间不够一个整块，那么整个空闲块都分配出去
        //
