...
cluster_t read_fat_entry(cluster_t clus)
{
    ...
    // ================== Your code here =================
    // void *fat16_init() 这里面显示了 meta.cluster_size 其实是 WORD*BYTE*1
    sector_t clus_sec = meta.fat_sec +  (clus * 2 / PHYSICAL_SECTOR_SIZE); // 应该是物理扇区？
    size_t sec_off = (clus * 2) % PHYSICAL_SECTOR_SIZE; // 2.2 DONE: 请填写正确的扇区内偏移量。// 可以为4.1提供参考
    int tmp = sector_read(clus_sec, sector_buffer);
    if(tmp < 0){
        return tmp;
    }
    cluster_t ret; // = read_from_cluster_at_offset(clus, offset, sector_buffer, //?);
    memcpy(&ret, sector_buffer + sec_off, sizeof(cluster_t));

    // ===================================================
    return ret; // DONE: 记得删除或者修改这一行
}
...
int find_entry_in_sectors(const char* name, size_t len, 
    ...
    for (size_t i = 0; i < sectors_count; i++) {
        int ret = sector_read(from_sector + i, buffer);
        // 对该扇区中的每一个目录项，检查是否是待查找的目录项（注意检查目录项是否合法）
        for (size_t offset = 0; offset < meta.sector_size; offset += DIR_ENTRY_SIZE) {
            DIR_ENTRY* dir = (DIR_ENTRY*)(buffer + offset);
            // 如果是待查找的目录项，将该目录项的信息填入 slot 中，并返回 FIND_EXIST
            ...
            // 如果不是待查找的目录项，检查该目录项是否为空，如果为空，将该目录项的信息填入 slot 中，并返回 FIND_EMPTY
            if (de_is_free(dir)) {
                slot->dir = *dir;
                slot->sector = from_sector + i;
                slot->offset = offset;
...
int find_entry_internal(const char* path, DirEntrySlot* slot, const char** remains) {
    ...
    *remains = next_level;
    while (true) {
         ...
         * Hint: 补全以下代码，实现对非根目录的路径的查找。
         * 提示，你要写一个 while 循环，依次搜索当前目录对应的簇里的目录项。
         */
        // ================== Your code here =================
        while (is_cluster_inuse(clus)) { 
                // 类比根目录查找方法，cluster_first_sector(clus) -- 开始搜索的扇区; 
...
int fill_entries_in_sectors(sector_t first_sec, size_t nsec, fuse_fill_dir_t filler, void* buf) {
    ...
    for(size_t i=0; i < nsec; i++) {
        ...
        sector_t sec = i + first_sec; // DONE: 请填写正确的扇区号
        int ret = sector_read(sec, sector_buffer);
        if(ret < 0) {
            return -EIO;
        }
        // 每个扇区中的所有目录项 // 
        for(size_t off = 0; off < meta.sector_size; off += DIR_ENTRY_SIZE ) { // DONE: 请补全循环条件（每个扇区多大？目录项多大？）
            DIR_ENTRY* entry = (DIR_ENTRY*)(sector_buffer + off);
            if(de_is_valid(entry)) {
                int ret = to_longname(entry->DIR_Name, name, sizeof(name)); // DONE: 请调用 to_longname 函数，将 entry->DIR_Name 转换为长文件名，结果存放在 name 中。
                if(ret < 0) {
                    return ret;
                }
...
int fat16_readdir(const char *path, void *buf, fuse_fill_dir_t filler, off_t offset, 
    ...
    if(path_is_root(path)) {
         ...
         */
        sector_t first_sec = meta.root_sec; // DONE: 请填写正确的根目录区域开始扇区号，你可以参考 meta 的定义。
        size_t nsec = meta.root_sectors; // DONE: 请填写正确的根目录区域扇区数
...
int read_from_cluster_at_offset(cluster_t clus, off_t offset, char* data, size_t size) {
     ...
     */
    uint32_t sec = cluster_first_sector(clus) + offset / meta.sector_size; // DONE?: 请填写正确的扇区号。
    size_t sec_off = offset % meta.sector_size; // DONE: 请填写正确的扇区内偏移量。// 可以为4.1提供参考
    ...
    while(pos < size) { // 还没有读取完毕
        ...
        // DONE?: 使用 memcpy 挪数据，从 sec_off 开始挪。挪到 data 中哪个位置？挪多少？挪完后记得更新 pos （约3行代码）
        // ================== Your code here =================
...
int fat16_read(const char *path, char *buffer, size_t size, off_t offset,
    ...
    while (is_cluster_inuse(clus) && p < size) {
        int read_size = min(meta.cluster_size, size - p); // 到结尾，不到结尾
...
int dir_entry_write(DirEntrySlot slot) {
    ...
    char sector_buffer[MAX_LOGICAL_SECTOR_SIZE];
    int ret = sector_read(slot.sector, sector_buffer); // DONE: 使用 sector_read 读取扇区
    if(ret < 0) {
        return ret;
    }
    memcpy(sector_buffer + slot.offset, &(slot.dir), DIR_ENTRY_SIZE); 
    // DONE: 使用 memcpy 将 slot.dir 里的目录项写入 buffer 中的正确位置;memcpy的参数是指针！
    ret = sector_write(slot.sector, sector_buffer); // DONE: 使用 sector_write 写回扇区
    if(ret < 0) {
        return ret;
    }
    return 0;
...
int dir_entry_create(DirEntrySlot slot, const char *shortname, 
    ...
    // ================== Your code here =================
    memcpy(dir->DIR_Name, shortname, 11); // go to definition //
    dir->DIR_Attr = attr;
    dir->DIR_FstClusHI = 0;
...
int write_fat_entry(cluster_t clus, cluster_t data) {
    ...
    size_t sec_off = clus_off % meta.sector_size; // 这是 4.1 的提示啊 orz 怎么这样
    for(size_t i = 0; i < meta.fats; i++) {
         ...
         */
        // ================== Your code here =================
        sector_t i_fat_sec = i * meta.sec_per_fat + meta.fat_sec + clus_sec; // 前+开始+簇内
        int ret = sector_read(i_fat_sec, sector_buffer);
        if(ret < 0){
            return ret;
        }
        memcpy(sector_buffer + sec_off, &data, sizeof(cluster_t));
        ret = sector_write(i_fat_sec, sector_buffer);
        if(ret < 0){
            return ret;
        }
        // 简直和 3.2 一模一样
...
int alloc_one_cluster(cluster_t* clus) {
     ...
     *              (?什么意思) （update:问了助教这一行没用）
     */

    // ================== Your code here =================
    int find = 0;
    for(cluster_t clus_i = CLUSTER_MIN; clus_i <= CLUSTER_MAX; clus_i++){
        if(read_fat_entry(clus_i) == CLUSTER_FREE){
            find = 1;
            int ret = write_fat_entry(clus_i, CLUSTER_END);
            if(ret < 0){
                return ret;
            }
            ret = cluster_clear(clus_i);
            if(ret < 0){
                return ret;
            }
    ...
    // ===================================================
    if(!find) return -ENOTSUP;    // TODO 请删除这一行或者修改为正确的返回值
}
...
int alloc_clusters(size_t n, cluster_t* first_clus) {
    ...
    for(cluster_t clus_i = 2 ; clus_i < meta.clusters; clus_i++){ // 需要注意前两个簇存 FF F7（？）不能用
        if(read_fat_entry(clus_i) == CLUSTER_FREE){
            clusters[free_clusters_num++] = clus_i;
        }
    ...
    if(free_clusters_num < n){ // 空闲的簇不够n个
        free(clusters);
        return -ENOSPC;
    }

    clusters[free_clusters_num] = CLUSTER_END; // 将最后一个簇连接至 CLUSTER_END
...
int fat16_mkdir(const char *path, mode_t mode) {
    ...
    const char* filename = NULL;
    cluster_t dir_clus = 0; // 新建目录的簇号 // 区别 mknod
    int ret = 0;
...
int fat16_unlink(const char *path) {
    ...
    ret = free_clusters(dir->DIR_FstClusLO); // 释放占用的簇
    if(ret < 0){
        return ret;
    }
    ...
    ret = dir_entry_write(slot);  // 写回目录项，写入文件系统
    if(ret < 0){
        return ret;
    }
    
    // ===================================================
    return 0; // DONE: 请修改返回值
}
...
int fat16_rmdir(const char *path) {
    ...
    // ================== Your code here =================
    DirEntrySlot slot;
    DIR_ENTRY* dir = &(slot.dir); // 仿照 unlink
    int ret = find_entry(path, &slot);
    if(ret < 0){
        return ret;
    }
    ...
    char name[MAX_NAME_LEN];
    const char DOT_NAME[] = ".          "; 
    const char DOTDOT_NAME[] = "..         "; // 据说是因为 . 和 .. 也在空目录中（并不是真的“空”，要排除这两个）
    cluster_t clus = dir->DIR_FstClusLO;
    while (is_cluster_inuse(clus)) { // 对于每个簇，查看所有的扇区
        sector_t first_sec = cluster_first_sector(clus);
        size_t nsec = meta.sec_per_clus;
        for(size_t i = 0; i < nsec; i++){
            sector_t sec = first_sec + i;
            int _ret = sector_read(sec, sector_buffer);
            if(_ret < 0){
                return _ret;
            }
...
ssize_t write_to_cluster_at_offset(cluster_t clus, off_t offset, const char* data, size_t size) {
    ...
    while(pos < size) { 
        ...
        int _ret = sector_write(sec, sector_buffer);
        if(ret < 0){
            return ret;
        }
        pos += len;        
...
int fat16_write(const char *path, const char *data, size_t size, off_t offset,
     ...
     * 1. 找到要写的文件对应的目录项（这样我们就知道文件在哪个簇。） 
        2. 比较新写入的数据和文件原来的数据所需的簇数量，如果新写入的数据所需的簇数量大于原来所需
        的簇数量，则需要为文件扩容簇数量。即在FAT表中查找未分配的簇，并链接在文件末尾。我们在
        新建目录时介绍了分配单个簇的过程，但文件写入时可能要新建多个簇，这个过程将在下面详细叙
        述。
        3. 按需扩容后，只需要实现写文件操作即可，获取第一个簇号，之后可以通过链接依次遍历，找到要
        写入簇的位置，并在读取相应的扇区，修改扇区的正确位置，并写回扇区。（这个过程和
        read 的实现很类似。）
        4. 更新对应的目录项，上述过程中，可能修改了目录项的某些部分，所以我们要相应更新目录项，然
        后将更改后的目录项写回镜像文件。
        5. 返回成功写入的数据字节数。
     * 
     * 
    ...
    */
    ...
    if(offset + size > clus_num * meta.cluster_size){
        ...
        ret = alloc_clusters(add_clus_num, &new_first_clus);
        if(ret < 0){
            return ret;
        }
    ...
    while(p < size){
        size_t len;
        len = min(size - p , meta.cluster_size - offset)
    ...
    // ===================================================
    return size; // DONE: 请修改返回值 -- 返回成功写入的字节数
}
